...
/** Score minimal pour accepter un match sémantique en live (paraphrases). */
const SEMANTIC_LIVE_MIN_SCORE = 42;

function passesLiveRelevance(verse, detectionMinPercent) {
  const source = verse.source;
  if (source === "reference" || source === "chapter") return true;
  if (source === "semantic") {
    return (verse.score || 0) >= SEMANTIC_LIVE_MIN_SCORE;
  }
  return (verse.score || 0) >= (detectionMinPercent || 0);
}
...
export default function App() {
  ...
  const [needsBibleInstall, setNeedsBibleInstall] = useState(false);
  const [bibleImporting, setBibleImporting] = useState(false);
  const [bibleImportMessage, setBibleImportMessage] = useState("");
  const [embeddingHint, setEmbeddingHint] = useState("");
  const [needsEmbeddingBuild, setNeedsEmbeddingBuild] = useState(false);
  const [embeddingBuilding, setEmbeddingBuilding] = useState(false);
  const [embeddingBuildMessage, setEmbeddingBuildMessage] = useState("");
  const [clientBundleReady, setClientBundleReady] = useState(false);
  ...
  useEffect(() => {
    ...
    fetch(apiUrl("/health"))
      ...
      .then((data) => {
        ...
        setNeedsBibleInstall(
          Boolean(data?.needsBibleImport) || (data?.bibleVersionsAvailable || 0) === 0
        );
        setNeedsEmbeddingBuild(Boolean(data?.needsEmbeddingBuild));
        setClientBundleReady(Boolean(data?.clientBundleReady));
        if (!data?.embeddingIndexReady && data?.semanticSearch !== "off" && !data?.clientBundleReady) {
          setEmbeddingHint(
            (prev) =>
              prev ||
              "Index sémantique absent — requis pour détecter les citations sans référence en live."
          );
        } else if (data?.embeddingIndexReady) {
          setEmbeddingHint("");
          setNeedsEmbeddingBuild(false);
        }
        if (data?.embeddingBuildRunning) {
          setEmbeddingBuilding(true);
          void pollEmbeddingBuildStatus()
            .then((done) => {
              if (done?.indexReady) {
                setEmbeddingHint("");
                setNeedsEmbeddingBuild(false);
            ...
            .finally(() => setEmbeddingBuilding(false));
        }
        if (data?.bibleImportRunning) {
          setBibleImporting(true);
          void pollBibleImportStatus()
            .then((done) => {
              if (done) return refreshBibleVersions();
            })
            .finally(() => setBibleImporting(false));
        }
      })
      .catch(() => {});
    ...
    fetch(apiUrl("/bible/versions"))
      ...
      .then((data) => {
        ...
        setNeedsBibleInstall(available.length === 0);
  ...
  async function refreshBibleVersions() {
    const r = await fetch(apiUrl("/bible/versions"));
    const data = await r.json();
    const available = (data?.versions || []).filter(
      (v) => v.available !== false && (v.verseCount || 0) > 0
    );
  ...
  async function pollBibleImportStatus() {
    const r = await fetch(apiUrl("/bible/import/status"));
    const data = await r.json();
    if (data?.progress?.message) setBibleImportMessage(data.progress.message);
    if (data?.running) return false;
    if (data?.error) throw new Error(data.error);
    return data?.result || true;
  }

  async function handleInstallBibles(slugs = []) {
    if (bibleImporting) return;
    setBibleImporting(true);
    setBibleImportMessage("Démarrage de l'import…");
    setError("");
    try {
      const r = await fetch(apiUrl("/bible/import"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(slugs.length ? { slugs } : {}),
      });
      const start = await r.json();
      if (!r.ok) throw new Error(start.error || "Import impossible.");

      let done = false;
      while (!done) {
        await new Promise((resolve) => setTimeout(resolve, 1200));
        done = await pollBibleImportStatus();
      }

      const { available, data } = await refreshBibleVersions();
      const slug =
        localStorage.getItem(LS_BIBLE_KEY) &&
        available.some((v) => v.slug === localStorage.getItem(LS_BIBLE_KEY))
          ? localStorage.getItem(LS_BIBLE_KEY)
          : data?.active || available[0]?.slug;
      if (slug) await applyBibleVersion(slug, available, data?.activeName);
      setNeedsBibleInstall(false);
      setBibleImportMessage("");
    } catch (e) {
      setError(`Bibles : ${e.message}`);
    } finally {
      setBibleImporting(false);
    }
  }

  async function pollEmbeddingBuildStatus() {
    const r = await fetch(apiUrl("/bible/embeddings/status"));
    const data = await r.json();
    if (data?.progress?.message) setEmbeddingBuildMessage(data.progress.message);
    if (data?.running) return false;
    if (data?.error) throw new Error(data.error);
    return data?.result || { indexReady: Boolean(data?.embeddingIndexReady) };
  }

  async function handleBuildEmbeddings() {
    if (embeddingBuilding) return;
    setEmbeddingBuilding(true);
    setEmbeddingBuildMessage("Démarrage…");
    setError("");
    try {
      const r = await fetch(apiUrl("/bible/embeddings/build"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          slugs: bibleVersion ? [bibleVersion] : [],
        }),
      });
      const start = await r.json();
      if (!r.ok) throw new Error(start.error || "Génération impossible.");

      let done = false;
      while (!done) {
        await new Promise((resolve) => setTimeout(resolve, 1500));
        done = await pollEmbeddingBuildStatus();
      }
      ...
      setEmbeddingBuildMessage("");
    } catch (e) {
      setError(`Index sémantique : ${e.message}`);
    } finally {
  ...
  async function applyBibleVersion(slug, versionsList = bibleVersions, fallbackName) {
    ...
    try {
      ...
      const data = await r.json();
      if (!r.ok) {
        if (data?.needsBibleImport) setNeedsBibleInstall(true);
        throw new Error(data.error || "Version introuvable.");
      }
      ...
      if (data.embeddingHint) {
        setEmbeddingHint(data.embeddingHint);
        setNeedsEmbeddingBuild(true);
      } else if (data.embeddingIndexReady) {
        setEmbeddingHint("");
        setNeedsEmbeddingBuild(false);
      }
  ...
  function pushDetections(phrase, suggestions, mode) {
    ...
    for (const verse of suggestions.slice(0, 3)) {
      ...
      if (!passesLiveRelevance({ ...verse, source }, config.detectionMinPercent)) {
        continue;
      }
      fresh.push({
    ...
    try {
      ...
      if (suggestions.length) {
        ...
        const pertinent = suggestions.filter((v) =>
          passesLiveRelevance(v, config.detectionMinPercent)
        );
        if (pertinent.length) {
    ...
    <div className="app">
        ...
        <div className="topbar-right">
          <div className="topbar-bible">
            ...
            <select
              ...
              value={bibleVersion}
              disabled={bibleVersionLoading || bibleImporting || !bibleVersionOptions.length}
            ...
            {(needsBibleInstall || bibleImporting) && !clientBundleReady && (
              <button
                type="button"
                className="bible-install-btn"
                disabled={bibleImporting}
                onClick={() => void handleInstallBibles()}
                title="Télécharge les versions bibliques (Louis Segond, Darby, etc.)"
              >
                {bibleImporting ? "Installation…" : "Installer les Bibles"}
              </button>
            )}
          </div>
          <div
      ...
      {embeddingHint && (
        <div className="embedding-hint-banner" role="status">
          <span>{embeddingHint}</span>
          {(needsEmbeddingBuild || embeddingBuilding) && !clientBundleReady && (
            <button
              type="button"
              className="embedding-build-btn"
              disabled={embeddingBuilding || bibleImporting}
              onClick={() => void handleBuildEmbeddings()}
              title="Génère l'index pour reconnaître les citations par le sens (sans référence biblique)"
            >
              {embeddingBuilding ? "Génération…" : "Générer l'index sémantique"}
            </button>
          )}
        </div>
      )}

      {embeddingBuildMessage && (
        <div className="embedding-build-status" role="status">
          {embeddingBuildMessage}
        </div>
      )}
      ...
      <main className="main board">
            ...
            <div className="panel-head">
              ...
              <div className="detections-head-actions">
                <label
                  ...
                  title="Seuil de pertinence pour les citations textuelles ; les références (« Jean 3:16 ») et paraphrases fortes passent toujours"
                >
                  Pertinence ≥
                  <input
                    type="range"
        ...
        {error && (
          <div className="error">
            <span>⚠ {error}</span>
            {(needsBibleInstall || /import-bibles|Installer les Bibles/i.test(error)) &&
              !clientBundleReady && (
              <button
                type="button"
                ...
                onClick={() => void handleInstallBibles()}
              >
                {bibleImporting ? "Installation en cours…" : "Installer les Bibles"}
              </button>
            )}
          </div>
        )}
        ...
        {bibleImportMessage && (
          <div className="bible-import-status" role="status">
            {bibleImportMessage}
          </div>
        )}
